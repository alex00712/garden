<template>
    <div v-if="isAdmin" class="row">
        <div class="col">
            <h3 class="head-line">Садовые растения, семена, цветы <br/> и многое другое, что делает этот <br/> мир
                лучше!</h3>
        </div>
    </div>
    ...
    <div v-if="!number && !isEdit" class="row justify-content-between">
        <div class="col-6 mb-4 mt-4">
            <button v-on:click="goTo('/')" type="button" class="btn btn-success"><i class="fas fa-home"></i> Домой
            </button>
        </div>
        <div class="col-6 mb-4 mt-4">
            <button v-on:click="goTo('/cart')" type="button" class="btn btn-success"><i
                    class="fas fa-shopping-cart"></i> Корзина
            </button>
        </div>
    </div>
    ...
    <div class="row" id="catalog">
        <div class="col-12 col-md-9 offset-md-3 col-lg-10 offset-lg-2">
            <div class="input-group mb-3">
                <input v-model="allFilters.search" type="text" class="form-control" name="search" @input="setFilters"
                       placeholder="Поиск" aria-label="Поиск" aria-describedby="basic-addon2">
    ...
    <div class="row">
                ...
                <div class="col-12">
                    <ul class="nav flex-row justify-content-center flex-md-column align-items-start">
                        <li class="nav-item d-flex justify-content-between align-items-center">
                            <a class="nav-link text-dark font-weight-bold cursor-pointer" name="all"
                               @click="handleChange">Все</a>
        ...
        <div ref="focus_list" class="col-12 col-md-9 col-xl-10">
            ...
            <div v-if="!isProductsAreFetching && notFull.forShowNow.length===0" style="margin-top: 100px"
                 class="d-flex justify-content-center">
                <h3>Ничего не найдено...</h3>
            </div>

            <transition-group v-else name="flip-list" class="row" tag="div">

                <div v-for="product in notFull.forShowNow"
                     class="c-flip-list-item col-12 mb-3 col-sm-6 col-lg-4 col-xl-3 flip-container" :key="product.id">

                    <div class="card d-flex flex-column justify-content-between my-card m-2 flipper">
                        <div class="front">
                            <img :src="loadImage(product.image)" class="card-img-top card-img-bottom img " alt="..."
                                 @click="openCardInfo(product.id)">
                            <div class="product-name-background">
                                <p class="font-weight-bold product-name product-name-front">{{product.name}}</p>
                                <p class="product-category">{{product.category.name}}</p>
                                <p class="product-family">{{product.family.name}}</p>
                            </div>
                            <div class="in-card-block" v-if="countExistingInMyCard(product.id)">
                                В корзине {{countExistingInMyCard(product.id)}} шт.
                            </div>
                        ...
                        <div class="back">
                            ...
                            <div v-if="!getNewLoader">
                                <transition v-if="!isEdit" name="buttons" mode="out-in">
                                    <div v-if="countExistingInMyCard(product.id)" key="yet"
                                    ...
                                    <div v-else key="notyet" class="d-flex flex-column m-1">
                                        <button style="visibility: hidden" class="btn btn-success mb-2"
                                                @click="buyNow(product)">Купить <i class="fas fa-dollar-sign"></i>
                                        </button>
                                        <button class="btn btn-outline-success mb-2"
                                                @click="addNewCardPosition(product)">Добавить <i
                                ...
                                <div v-else class="d-flex flex-column m-1">
                                    <button class="btn btn-outline-primary mb-2"
                                            @click="goTo(`/admin/editor/${product.id}`)"><i class="fas fa-cog"></i>
                                        Редактировать
                                    </button>
                                    <button class="btn btn-outline-danger mb-2" @click="deleteProduct(product.id)">
                                        <i
                                                class="fas fa-edit"></i> Удалить
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div v-if="notFull.amuuntOfCeilPagination && !isProductsAreFetching" class="col-12 mt-4">
                    <nav aria-label="Навигация по страницам">
                        <ul class="pagination justify-content-center">
                            ...
                            <li v-for="i in notFull.amuuntOfCeilPagination" :class="{'active': i===paginationNamber}"
                                class="page-item m-2" :key="i">
...
<style lang="css" scope>
    ...
    .product-name-background {
        ...
        width: 100%;
        height: 60px;
        background: linear-gradient(45deg, #039a0e, transparent);
    }
